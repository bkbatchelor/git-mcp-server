 ...
 * For any valid Git repository with staged changes, the git_commit tool should create a commit
 * with the provided message and return commit information, and return appropriate errors for
...
class GitCommitOperationsProperties {
    ...
    @Tag("property-5")
    @Tag("git-commit-operations")
    void gitCommitReturnsErrorForRepositoryWithoutStagedChanges(@ForAll("repositoryWithoutStagedChanges") Path repositoryPath,
                                                               @ForAll("validCommitMessages") String commitMessage) {
        // Arrange
        GitCommitTool tool = new GitCommitTool();
    ...
    @Tag("property-5")
    @Tag("git-commit-operations")
    void gitCommitReturnsErrorForInvalidCommitMessage(@ForAll("repositoryWithStagedChanges") RepositoryWithChanges repoState,
    ...
    Arbitrary<String> invalidCommitMessages() {
        return Arbitraries.oneOf(
            ...
            Arbitraries.just("\n\n\n"), // Newlines only
            Arbitraries.strings().withCharRange('\u0000', '\u001F').ofMinLength(1).ofMaxLength(10) // Control characters
        );
    }
